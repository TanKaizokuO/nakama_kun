...
class CoderAgent(BaseAgent):
    """Coder Agent performs implementation and writes code in the workspace using tools."""
    ...
    def __init__(
        ...
        if tool_router is None:
            from nakama_kun.tools import ToolRouter
        ...
        self.memory["implementation_history"] = []

    @property
    def implementation_history(self) -> list[Any]:
        """Returns the history of implementations performed by the coder."""
    ...
    async def execute(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if goal_satisfied:
            logger.info("[CoderAgent] Goal is already satisfied. Skipping execution.")
            return {
                "messages": [],
                "tool_results": [],
        ...
        for round_idx in range(1, max_rounds + 1):
            if goal_satisfied:
                break
            logger.info(f"[CoderAgent] Coder Round {round_idx}/{max_rounds}...")
            ...
            if response.finish_reason == "stop" or not response.tool_calls:
                ...
                if missing_artifacts and not delivery_mode:
                    delivery_mode = True
                    logger.info("[CoderAgent] LLM stopped before required artifacts; forcing delivery mode.")
                    continue
                logger.info("[CoderAgent] Execution completed.")
                break
            ...
            for tc in response.tool_calls:
                ...
                logger.info(f"[CoderAgent] Dispatching tool {final_name} with args: {final_arguments}")
                error: str | None = None
                ...
                if block_reason:
                    ...
                    logger.warning(f"[CoderAgent] ToolSelectionLayer blocked: {block_reason}")
                    ...
                    content = result.to_content()
                    logger.warning(
                        f"[CoderAgent] Blocked exploratory tool call {final_name} in delivery mode."
                    )
                elif (
                    ...
                    logger.warning(
                        f"[CoderAgent] Blocked repeated failed tool call {final_name} "
                    ...
                    try:
                        ...
                        logger.error(f"[CoderAgent] Tool execution failed: {exc}")
                ...
                if success and not goal_satisfied:
                    ...
                    if gsr.goal_satisfied:
                        ...
                        logger.info(
                            f"[CoderAgent] GoalSatisfactionDetector: {gsr.explanation} "
            ...
            if goal_satisfied:
                logger.info("[CoderAgent] Goal satisfied. Terminating execution early.")
                break
        ...
        return {
            ...
            "coder_proposals": [],  # compatible legacy empty proposals list
        }
